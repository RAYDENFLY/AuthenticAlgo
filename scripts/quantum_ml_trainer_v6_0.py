...
class QuantumMLTrainerV60:
    ...
    async def get_data(self, symbol: str, limit: int = 1000) -> pd.DataFrame:
        """Get data with extended history using safe collector"""
        logger.info(f"📊 Collecting data for {symbol} (limit={limit})...")
        klines = await self.collector.exchange.get_klines_safe(symbol, '1h', limit)
    ...
    async def train_v60(self, symbol: str) -> Dict:
        ...
        X_dl_combined = np.hstack([tcn_features, attention_features])
        logger.info(f"      Before selection: {X_dl_combined.shape}")
        try:
            logger.info("   🎯 Applying feature selection...")
            from sklearn.feature_selection import SelectKBest, f_classif
            selector = SelectKBest(score_func=f_classif, k=min(150, X_dl_combined.shape[1]))
            logger.info("   Fitting selector...")
            selector.fit(X_dl_combined, y)
            logger.info("   Transforming features...")
            X_dl_selected = selector.transform(X_dl_combined)
            logger.info(f"      After selection: {X_dl_selected.shape}")
        except Exception as e:
            logger.error(f"   ❌ Feature selection failed: {e}")
            logger.info("   Using fallback: first 150 features")
            X_dl_selected = X_dl_combined[:, :150]
            selector = None
        X_combined = np.hstack([X_base.values, X_dl_selected])
        logger.info(f"      Final features: {X_combined.shape}")
        ...
        # Expose feature pipeline artifacts for downstream demo/backtest
        results['base_feature_cols'] = base_feature_cols
        # Store final feature columns (base + selected deep features)
        final_feature_cols = list(X_base.columns) + [f"dl_{i}" for i in range(X_dl_selected.shape[1])]
        results['final_feature_cols'] = final_feature_cols

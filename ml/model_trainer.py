...
try:
    ...
    TF_INSTALLED = False
    logger = get_logger()
    logger.warning("TensorFlow not installed, skipping LSTM models")
...
class ModelTrainer:
    ...
    def __init__(self, config: dict):
        ...
        self.hyperparameter_optimization = self.training_config.get('hyperparameter_optimization', False)
        self.gpu_memory_limit = self.training_config.get('gpu_memory_limit_mb', None)
        
        # Setup GPU memory limit for TensorFlow if configured
        if self.gpu_memory_limit and TF_INSTALLED:
            self._setup_gpu_memory()
        
        # GTX 1050 Ti: Log optimization settings
        if 'lstm' not in self.model_types:
            self.logger.info("🎯 GTX 1050 Ti Mode: LSTM disabled for memory optimization")
        if not self.hyperparameter_optimization:
            self.logger.info("🎯 GTX 1050 Ti Mode: Hyperparameter optimization disabled")
    ...
    def _setup_gpu_memory(self):
        """Setup GPU memory limits for GTX 1050 Ti"""
        try:
            ...
            if gpus:
                ...
                if self.gpu_memory_limit:
                    ...
                    self.logger.info(f"GPU memory limited to {self.gpu_memory_limit}MB")
        except Exception as e:
            self.logger.warning(f"Could not setup GPU memory: {e}")
    ...
    def _train_xgboost(self, X_train: pd.DataFrame, X_test: pd.DataFrame, 
        ...
        # Update with config if provided (GTX 1050 Ti optimization)
        xgb_config = self.training_config.get('xgboost_params', {})
        params.update(xgb_config)
        
        # GTX 1050 Ti: Use GPU if configured
        if 'tree_method' in xgb_config and xgb_config['tree_method'] == 'gpu_hist':
            self.logger.info("🎯 Using GPU acceleration for XGBoost")
        
        model = xgb.XGBRegressor(**params)
        
        # Hyperparameter optimization (skip for GTX 1050 Ti optimization)

"""
Run Technical Analysis Strategy Only
...
async def run_technical_strategy():
    """Run Technical Analysis strategy only"""
    
    logger.info("=" * 80)
    logger.info("📊 TECHNICAL ANALYSIS STRATEGY")
    logger.info("=" * 80)
    logger.info("")
    logger.info("Strategy: Technical Analysis (RSI + MACD)")
    ...
    # Initialize trader
    trader = TechnicalAnalysisTrader(capital=10.0)
    
    # Run competition
    logger.info("🚀 Starting Technical Analysis strategy...\n")
    ...
    # Print results
    logger.info("\n" + "=" * 80)
    logger.info("📊 TECHNICAL ANALYSIS - FINAL RESULTS")
    logger.info("=" * 80)
    logger.info(f"\n💰 Capital:")
    ...
    # Save JSON
    json_file = report_dir / f"technical_results_{timestamp}.json"
...
if __name__ == "__main__":
    asyncio.run(run_technical_strategy())

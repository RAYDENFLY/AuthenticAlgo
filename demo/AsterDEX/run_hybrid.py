...
async def run_hybrid_strategy():
    ...
    logger.info("=" * 80)
    logger.info("")
    logger.info("Strategy: Hybrid (TA + ML combined)")
    ...
    # Initialize trader
    trader = HybridTrader(capital=10.0)
    
    # Run competition
    logger.info("🚀 Starting Hybrid strategy...\n")
    ...
    # Print results
    logger.info("\n" + "=" * 80)
    logger.info("📊 HYBRID STRATEGY - FINAL RESULTS")
    logger.info("=" * 80)
    logger.info(f"\n💰 Capital:")
    ...
    # Save JSON
    json_file = report_dir / f"hybrid_results_{timestamp}.json"
    with open(json_file, 'w') as f:
        json.dump(result, f, indent=2, default=str)
    
    logger.info(f"\n💾 Report saved: {json_file.name}")
    logger.info("\n" + "=" * 80)
    logger.info("✅ HYBRID STRATEGY COMPLETE!")
    logger.info("=" * 80)
    logger.info("")


if __name__ == "__main__":
    asyncio.run(run_hybrid_strategy())
